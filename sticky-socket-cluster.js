...
module.exports = function pool (options, callback) {
  ...
  var session_hash_ip = function (req, res) {
    // get IP from headers (Nginx or other proxy) or connection
    var ip = (req.headers['x-real-ip'] || req.headers['x-forwarded-for'] || req.connection.remoteAddress || '').split(/\./g)
  ...
  if (cluster.isMaster) {
    ...
    cluster.on('exit', function (worker, code, signal) {
      debug_error('Worker died (ID: ' + worker.id + ', PID: ' + worker.process.pid + '), port: ' + worker.port)
    ...
    setTimeout(function () {
      require('./proxy').init(options.workers, options.first_port, options.proxy_port, options.session_hash,
        options.no_sockets)
